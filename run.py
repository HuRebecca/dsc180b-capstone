...
DESCRIPTION: This file when run will execute the whole project from the data ingestion process
  to the analysis. Right now, however, it just has the data ingestion.
'''
...
def create_csv_of_actors():
    ...
    for row in table.find_all('tr'):
        ...
        with open("/home/pvarkhed/180B/actorsTemp.csv",'a+', newline='') as outF:
...
def add_ethnicity_to_nominees_and_winners()
    ...
    table.head(10)
    table.to_csv(r'/home/pvarkhed/180B/actorsWithEthnicity.csv', index = False)
...
def create_csv_per_year(tabNum, year):
    ...
    for tab in soup.find_all('table')[tabNum:]:
        ...
        for i in td_tags:
            ...
            if count == 5:
                lst = [title, actors]
                with open("/home/pvarkhed/180B/{}.csv".format(year),'a+', newline='') as outF:
                    writer = csv.writer(outF, dialect='excel')
                    writer.writerow(lst)
...
INPUTS: fp: files path to the config file

OUTPUTS: param: json file of the paramerters from config file

DESCRIPTION: THis function loads the necessary argument for the run.py file
'''
def load_params(fp):
    with open(fp) as fh:
        param = json.load(fh)
...
INPUTS: targets: target from command line to run

OUTPUTS: None (runs file)

DESCRIPTION: This funtion runs the necessary code in the file
    according to the targets speciified
'''
def main(targets):

    #if block to scrape all the winners and nominess from each year since 1934 for Best Actor
    if 'getNomineesAndWinners' in targets:
        create_csv_of_actors()
        
        #####TODO - edits actors file####
        
        add_ethnicity_to_nominees_and_winners()
     
    #if block to scrape all movies made and actors in the for each year from 1934
    if 'getAllPerYear' in targets:
        all_movies_per_year()
